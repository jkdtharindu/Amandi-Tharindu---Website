...
**B. The 5 static public pages — cached, and explicitly busted on save.**
Home, Our Story, The Celebration, Gallery, and Wishes are pre-rendered as static HTML for speed. If the admin changes the theme, a page section, or a celebration event, that change would otherwise only reach guests on the next full deploy. To avoid that, every admin save to Theme, Sections, or Events explicitly calls `revalidateAllPublicPages()` ([src/revalidatePublicPages.js](src/revalidatePublicPages.js)), which tells Next.js to throw away and regenerate the cached HTML for those 5 specific paths. The next guest to load any of those 5 pages gets the new content; anyone with the page already open in their browser needs to reload.
