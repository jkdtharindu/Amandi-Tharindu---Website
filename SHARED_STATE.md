...
|---|---|---|---|
| Admin creates/edits/deletes a guest | `POST` / `PATCH` / `DELETE` `/api/admin/guests[/id]` | `guests` (delete is soft: `isDeleted = true`). **Creating** a guest now requires named people (`validateGuestInput(..., { requirePeople: true })`): two or more seats with no names is refused (400, `errors.inviteeNames`), and one seat with no names is saved as one `Invitee` carrying the guest's own name — so every new party appears in Table Arrangement's picker once accepted. Editing does not require names (older guests are headcount-only). An edit saves the name, relationship and WhatsApp number, and for a party with named people replaces the submitted `slotCount` with the real count of approved Invitees — both in the same UPDATE (`src/admin/updateGuestDetails.js`), so the headcount can never be stale and the other fields can never be silently dropped. A delete also **frees every table seat** the guest or any of their people held, in one transaction (`src/admin/removeGuest.js`) — the table window reads seats with no `isDeleted` filter, so this is what keeps a removed guest off the seating chart. The unassigned-people list and the Balance to Arrange count skip a removed guest's people too (`listUnassignedInvitees`) | No |
...
| Admin approves/rejects a pending invitee request | `POST /api/admin/invitee-requests/[id]/approve` \| `reject` | Approve is **one transaction** (`src/invitees/approveInviteeRequest.js`): sets `approvalStatus: 'approved'`, adds one to the guest's `slotCount` and re-derives the party's `rsvpStatus` (an undecided new person can move a fully declined party back to `pending`); if any step fails none are kept and the admin gets a 500. Only a request still `pending_approval` is approved — a repeat gets 409 `not_pending`, and a removed guest's request gets 409 `guest_removed`. The approval list (`GET /api/admin/invitee-requests`) leaves out requests from removed guests (`src/invitees/pendingRequests.js`). Reject sets `'rejected'` and is unchanged | No |
...
| Admin assigns/unassigns a seat | `POST /api/admin/table-arrangement/[tableId]/seats/[seatId]/assign` \| `unassign` | `table_seats`. Assign is **refused (400)** if the seat already holds a *different* person — the seat has to be emptied first; re-assigning the seat's own occupant is still allowed. Unassign is never refused and wipes the seat's notes. **The Table Arrangement screen no longer shows or saves dietary requirements or notes** (Action 63) — the columns, the API's optional `dietaryRequirements`/`specialNotes` fields and the spreadsheet export still exist, so a value saved earlier stays in the database and the export until its seat is emptied. The seat list also returns `inviteeGuestName`, the party a seated invitee belongs to, which the screen shows as "Name (Party)". **The guest's own `/invitation/[code]` page shows the seat** (`getGuestTableView`, `src/table-arrangement/guestTableView.js`): under the name line, "Table N" and the first names of the *other* people at that table (own party and anonymous probable placeholders left out). It is read fresh on every load, so a new assignment shows on the guest's next page load — no cache to bust | No |
