...
- **Also canonical, docs/-only, both merged to `main` 2026-09-09:** `docs/VIBE_CODING_PRODUCTION_CHECKLIST.md` and `docs/DEPLOYMENT.md`.
  ...
  - `docs/DEPLOYMENT.md` is the deployment runbook. **Nothing in it has been executed against a real Vercel project**, and it says so at the top — treat it as derived from the code (it carries `file:line` references) rather than as proven procedure, and add what the first real deploy teaches. Its §1 is load-bearing: `NEXT_PUBLIC_SITE_URL` must be set before any guest is messaged, or every invitation link points at `localhost`.
