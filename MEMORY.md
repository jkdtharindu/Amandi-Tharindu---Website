...
[2026-08-28] Decision: Build the actual ThemePalette and FontChoice picker UI at `/admin/theme`, replacing the plain hex/font-name text inputs as the primary path, per PRD §4.1's 2026-08-23 requirement.
Reason: Owner asked directly ("let's build the actual palette picker") after seeing that approving "Modern Royal Romance" in the PRD table had no visible effect on the running site — the picker feature itself had never been built, only documented. Added `src/theme/palettes.js` (THEME_PALETTES, FONT_CHOICES data + lookup helpers), extended `mergeThemeUpdate` to cascade a `paletteName`/`fontChoice` selection into the underlying colour/font fields (and to clear that selection back to "Custom" if the underlying field is later hand-edited via the kept Advanced Colours/Advanced Font groups), added `palette_name`/`font_choice` to `themeStore`/`themeRepo`, wrote (not applied) migration `004_add_theme_palette_font_choice.sql`, and rendered swatch/font-option pickers with an instant client-side live preview in `src/server.js`. Verified in the browser: selecting "Modern Royal Romance" and saving cascades burgundy/ivory/gold to the public `/home` page's CSS custom properties. 12 new tests (`tests/theme-palettes.test.mjs`, `tests/theme-picker.test.mjs`, plus additions to `tests/theme-settings.test.mjs`) cover the WCAG AA gate for all six palettes and the cascade/detach/rejection behaviour; full suite at 78/78, `src/` lint-clean.
Alternative considered: Leaving the palettes as documentation-only until a larger Next.js migration (rejected — the owner asked for it now, and the existing dual-mode repo pattern made it a same-day fit for the current Express prototype, consistent with how the rest of the admin panel was built).
