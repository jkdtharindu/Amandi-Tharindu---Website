...
8. Security & production readiness (Model: Opus 5) — **partially done 2026-09-05**, split below.
   - [✔] Security headers on every response (2026-09-05). `src/security/securityHeaders.js` builds them; `next.config.ts` applies them to `/:path*` and sets `poweredByHeader: false`. CSP, X-Frame-Options DENY, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, X-DNS-Prefetch-Control, plus HSTS in production only (sending HSTS from a local `http://` dev server would pin the browser to https for localhost). Verified in a real browser against a production build: all six public pages render with zero CSP violations and zero console errors.
   - [✔] Rate limiting on both login endpoints (2026-09-05). `RateLimiter` ported from the archived branch and stripped of its Express coupling (`src/security/rateLimiter.js`); `src/security/authRateLimit.js` wires it into `/api/guest/login` (20 per 10 min — it had **no limit at all** before, and it is the brute-force surface for the entire guest list) and `/api/admin/login` (8 per 15 min, unchanged from the ad-hoc counter it replaced). A successful *guest* login deliberately does not refund the budget; a successful *admin* login does — except from the shared `unidentified` bucket, which is never refunded. Three defects in this slice were caught by a pre-merge review of its own diff and fixed before merge: cleanup expired history against a fixed 5-minute cutoff rather than each caller's window (which cut the 15- and 10-minute lockouts short), the admin route counted attempts only after parsing the body, and a success refunded the shared bucket. See MEMORY.md 2026-09-05 "Past mistakes".
   - [✔] Fixed a rate-limit bypass in the existing admin throttle (2026-09-05). It keyed on the **leftmost** `X-Forwarded-For` entry, which the caller controls — rotating that header handed out a fresh budget on every request. `src/security/clientIp.js` now counts back from the right by `TRUSTED_PROXY_COUNT` (documented in `.env.example`; defaults to 1 in production, 0 elsewhere). Verified end-to-end over HTTP that a rotating header no longer bypasses the limit.
   - [ ] Move login throttling to a shared store. Still in-process, so it resets on redeploy and is per-instance — fine for a single instance, not for more than one. Needs an infra decision (Redis vs Postgres); this project has no Redis today. (Model: Opus 5)
   - [ ] Add an `updated_at` column to `guests` — needs a migration, so HITL-gated. (Model: Sonnet 5)
   - [ ] Set `NEXT_PUBLIC_SITE_URL` to the real origin at deploy time (Next Action 10). (Model: Sonnet 5)
   - [ ] Privacy review for guest data before any public exposure: no retention/deletion policy exists yet, and Supabase RLS is still not enabled, so database-level access control rests entirely on the app's admin-auth layer. (Model: Opus 5)
   - [✔] Ported the sanitizing logger from `archive/claude/mobile-only-session-qj1ixz-2026-09-04` (2026-09-05). `src/security/securityLogger.js` writes one JSON line per security event and masks guest data at any depth (invitation code, name, email, WhatsApp number, tokens), plus IP masking and path sanitizing. Wired into `checkAuthRateLimit` (logs the *first* denial of a block only, so hammering cannot flood the logs) and both login routes (failed and successful attempts). **The archived branch's Express `requestLoggerMiddleware` was deliberately NOT ported** — it hangs off `res.on('finish')`/`req.ip`, which do not exist in the Next.js route handlers that serve production; per-request logging for the real app belongs at the platform edge. Its `maskIpAddress` (stray trailing space, no address validation), shallow `_sanitize`, and `formatBytes` (breaks above MB) were rewritten rather than copied. Verified live: 22 hammering requests produced exactly 1 block line, and a canary invitation code and guest name appeared nowhere in the server log.
   - [ ] That branch's RLS-policy migration and admin-auth scaffolding remain NOT reusable as-is — see MEMORY.md 2026-09-04 "Abandoned Branches". Nothing further to port from it. (Model: Opus 5)
