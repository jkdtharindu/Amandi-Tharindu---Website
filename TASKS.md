...
- Current focus: Security hardening, backups and the mobile review shipped 2026-09-06 (Next Actions 8, 9, 14). P1 remaining: the pre-login site gate (P1-15, still blocked on the same product questions — see Next Action 12).
- Priority: **Backup restore rehearsal (Next Action 14)** — the only open item whose failure mode is unrecoverable, and the one thing that turns "we have backups" from a claim into a fact. It needs a browser and a scratch database, so it is the owner's to do. Then P1-15 (needs three product decisions), then deployment prep (Next Action 10).
...
8. Security & production readiness — **partly done 2026-09-06** (Opus 5). Shipped: guest login rate limiting (10 attempts / 10 min / IP, in both the Next.js route and the Express prototype so the suite can test it over HTTP — 4 new tests), magic-byte image validation actually wired into `/api/admin/upload` (2 new tests; multer's `fileFilter` only ever saw the client's own Content-Type, so a renamed executable claiming `image/png` used to pass), and security headers via `proxy.ts`. Three corrections to the earlier attempt at this: the file was written as `middleware.ts`, which Next.js 16 **deprecated in favour of `proxy.ts`**; its CSP had no `unsafe-eval` in development, which silently broke React's dev overlay and stack traces; and it shipped `X-XSS-Protection`, now removed — the legacy XSS auditor is deprecated and was itself a vulnerability source, CSP replaces it. Verified live over HTTP, not just asserted: headers present on a real response, and the 11th login attempt actually returns 429. **Still open from this item:** move throttling to a shared store (the counter is per-process, so it resets on redeploy and does nothing across multiple instances — this is the single biggest caveat on the rate limiting), `updated_at` on `guests` (migration/HITL), `NEXT_PUBLIC_SITE_URL` for production, CSRF tokens have no TTL and are pure double-submit so they are not an authentication control, and the guest-data privacy review. Original note follows: move admin login throttling to a shared store, add an `updated_at` column to `guests` (needs migration/HITL), set `NEXT_PUBLIC_SITE_URL` for production, privacy review for guest data before any public exposure. Reusable pieces found 2026-09-04 in archived branch `archive/claude/mobile-only-session-qj1ixz-2026-09-04` (never merged): rate-limiting middleware (`src/rate-limiter.js`, tested) and security-headers middleware (`src/security-headers.js`) look directly portable; a request-logging/sanitization middleware (`src/request-logger.js`) is also there. That branch's RLS-policy migration and its own admin-auth scaffolding are NOT reusable as-is — see MEMORY.md 2026-09-04 "Abandoned Branches" for why. (Model: Opus 5)
...
14. Backups (added 2026-09-06, partly done). Application-level backup shipped and run: `npm run backup` writes every table to `backups/*.json` (tables discovered from the catalog, so a future migration cannot silently leave its table unbacked), `npm run backup:verify` reads the newest one back. `backups/` is gitignored — the files contain every guest's name, phone and email, so they must never be committed or put in a shared folder. First real backup taken against the live database (36 rows / 12 tables). **Three things still need the owner and a browser**, tracked as a checklist in `docs/BACKUP_RECOVERY.md`: (a) confirm Neon's point-in-time retention window, (b) rehearse one restore into a scratch branch, (c) choose an off-machine location for the backup files. Until (b) happens, "we have backups" is a claim, not a fact — this is the highest-priority open item in the project because it is the only one whose failure mode is unrecoverable. (Model: Opus 5)
15. Doc/tooling drift found 2026-09-06, filed rather than fixed (all three are code or deletion changes, out of scope for a docs pass): (a) **`npm run lint` is effectively unusable** — it reports 5,298 problems, of which roughly 5,290 come from the leftover `.claude/worktrees/focused-kilby-265cc0` folder; only about eight are in real source files. This is why a previous session could report "lint clean" while 383 errors existed. Needs an eslint ignore for `.claude/`. (b) **`.env.example` describes behaviour that does not exist** — it calls `NEXT_PUBLIC_SITE_URL` the "origin used for CSRF/Origin verification in proxy.ts", but that variable is read only by `app/admin/guests/page.tsx` and `app/admin/messages/page.tsx` to build WhatsApp invitation links, and no origin verification exists anywhere (`src/csrf.js` is a pure double-submit cookie compare that never inspects origin). (c) **`SECURITY_HARDENING_SUMMARY.md` sits at the repo root**, off-convention and overlapping this file; candidate to fold into TASKS.md/MEMORY.md and delete. (Model: Haiku 4.5 — mechanical, but (b) is a wording decision the owner may want to make)
