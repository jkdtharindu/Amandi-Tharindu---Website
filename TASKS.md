...
- Current focus: messaging center (P1-06/P1-07) shipped 2026-09-05. P1 remaining: the pre-login site gate (P1-15, blocked on two product questions — see Next Action 12) and the Table Arrangement Dashboard & Probable Attendance (P1-16, scoped 2026-09-05 via Grill Me session — see Next Action 13).
- Priority: Decide between P1-15/P1-16 (both need user/HITL sign-off before coding) and the production-readiness work in Next Actions 7–10
...
- Table Arrangement Dashboard & Probable Attendance (P1-16) — newly proposed feature, scoped 2026-09-05 via a Grill Me session; full spec in PRD §16. Not started; needs a HITL-approved migration (010) before coding.
- Still missing from PRD scope: full messaging center, the pre-login gate above, the Table Arrangement Dashboard above, production deploy.
...
- [ ] Table Arrangement Dashboard & Probable Attendance (P1-16, scoped 2026-09-05) — RSVP-funnel stats (Accepted / Table Arranged / BalanceToArrange / RSVP Not Accepted) on `/admin/table-arrangement`, plus an Admin-editable "probable attendance" buffer per Declined/Pending bucket that materializes anonymous `ProbableAttendee` seat placeholders (no name, no link to a real Guest, `rsvp_status` never touched). Full spec, schema, and open questions in PRD §16. New migration (010, additive-only) needs HITL approval before coding starts. Model: Sonnet 5 — proposed, not yet confirmed (well-scoped CRUD + one new join-safe constraint, no auth/security surface; escalate to Opus 5 only if the resize/concurrency logic around shrinking a buffer proves trickier than expected).
...
13. Table Arrangement Dashboard & Probable Attendance (P1-16) — scoped 2026-09-05 via a Grill Me session, not started. Adds RSVP-funnel stats and an anonymous `ProbableAttendee` buffer mechanism to `/admin/table-arrangement` — see PRD §16 and the new `probable_attendees`/`table_seats.probable_attendee_id` schema documented in `WEDDING_DATABASE_SCHEMA.md`. Open questions to resolve before coding (full list in PRD §16): exact placeholder label wording; whether ProbableAttendee resizing is a GroomAdmin-only (super admin) action given it isn't GuestPartition-scoped; confirming the headline stats stay strictly Accepted-Guest-only. Migration 010 is additive-only but still needs HITL approval before applying to any database. (Model: Sonnet 5 — proposed, not yet confirmed per the Model Assignment Convention)
