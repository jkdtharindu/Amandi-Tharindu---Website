...
10. Deployment prep: Vercel config, production env vars, HITL-gated deploy (Model: Sonnet 5) — **prepared 2026-09-05, deploy itself not run.**
    - `docs/DEPLOYMENT.md` written: every environment variable with what it does and where its value comes from, the deploy steps, post-deploy checks, and rollback. Nothing in it has been executed against a real Vercel project yet — it was derived from the code, with file:line references so each claim is checkable.
    - **Found while writing it:** `app/admin/guests/page.tsx:24` and `app/admin/messages/page.tsx:60` fall back to a hardcoded `http://localhost:3010` when `NEXT_PUBLIC_SITE_URL` is unset, and that value flows through `buildInvitationLink()` into the WhatsApp text sent to guests. Deploying without setting that variable would send **every guest a dead link**, with nothing failing or warning in the admin panel. Documented as §1 of the runbook. No code change made — the fallback is reasonable for local development; the fix is setting the variable at deploy time.
    - No `vercel.json` is needed; Vercel detects Next.js automatically.
    - Migrations are NOT applied by the deploy — `npm run migrate` is manual and HITL-gated. Live DB already has 001–010.
    - [ ] Remaining: the deploy itself, which needs the owner's Vercel account. (Model: Sonnet 5)
    - [ ] Remaining, and higher priority than the deploy: verify the guest-list backups can actually be restored. Losing the guest list near the wedding is a more realistic disaster than any attack, and nobody has tested a restore. (Model: Sonnet 5)
