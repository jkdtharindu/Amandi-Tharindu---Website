...
14. Backups (added 2026-09-06). Application-level backup shipped and run: `npm run backup` writes every table to `backups/*.json` (tables discovered from the catalog, so future migrations are picked up automatically), `npm run backup:verify` reads the newest one back. `backups/` is gitignored — the files contain every guest's name, phone and email. First real backup taken against the live database (36 rows / 12 tables). **Three things still need the owner and a browser**, tracked in `docs/BACKUP_RECOVERY.md`: confirm Neon's point-in-time retention window, rehearse one restore into a scratch branch, and choose an off-machine location for the backup files. Until the rehearsal happens, "we have backups" is untested. (Model: Opus 5)
