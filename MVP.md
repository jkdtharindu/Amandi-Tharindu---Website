...
- [ ] **Do not push `main` as it stands.** Three local commits (`67cb04e`, `3cfa574`, `987bea3`) hold the multi-admin work (a fourth, `40a2d09`, is the separate guest-edit fix in Next Action 74 — earlier notes counted it here by mistake). It needs migrations 020-022 run by you (`npm run migrate`, from your own terminal) and new bride/groom login settings in Vercel **before** the push — pushed first, the whole admin panel below login breaks on the live site — guest list, dashboard, opening or editing a guest, and Table Arrangement all 500 on the missing columns. Corrected 2026-09-21: **admin login itself does not break** (the legacy `ADMIN_EMAIL` fallback still works and signs in as the bride), and guest-facing pages are unaffected. The nine admin session tests that failed against it are **fixed as of 2026-09-21** (606 of 606 pass, Next Action 69), and local `main` has now **merged `origin/main`** (`cdba1fa`), so it is 20 ahead and 0 behind. Actions 55 and 58 shipped separately as PR #16 (merged and deployed 2026-09-21, `701cdc0`). **Only two things are left before the push, and both are yours:** run `npm run migrate` for 020-022 from your own terminal, then set `BRIDE_EMAIL`/`BRIDE_PASSWORD_HASH` and `GROOM_EMAIL`/`GROOM_PASSWORD_HASH` in Vercel (`npm run admin:set-password` generates the hashes). Pushing is still your call.
