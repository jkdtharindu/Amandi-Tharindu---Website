...
Test suite: 555/555 passing as of 2026-09-20 (everything through Action 57 is deployed, and Action 59's repository change went live inside commit `fb86121`; the last 27 tests are Actions 59-60's, pushed afterwards in `fe44873`).
...
- [ ] Action 59's seat rule (live inside `fb86121`): seating one person on an empty seat **worked on the live site** (owner-reported 2026-09-20). Still to check: open Table Arrangement in two tabs, seat someone in tab 1, and try to seat a different person on the same seat in tab 2 — you should see "That seat already has someone on it." and the plan should refresh.
- [ ] **After the Action 63 change is deployed:** on Table Arrangement, a seated person should show as "Name (Family or group name)", and the seat cards should have no dietary or notes boxes and no Save button. Seat two people who share a first name from different parties and confirm you can tell them apart.
- [ ] **After the Action 62 fix is deployed** (until then, this is still broken on the live site): open a guest who has named people, change their WhatsApp number or their name, save, then reload the page and confirm the change is still there. Check the Seats number still matches how many people are named. This was click-tested here against a stand-in database, never against the real one.
...
- [x] Pending "add another person" requests from a removed guest still showed for approval (Next Action 58, found 2026-09-20). *Fixed and click-tested 2026-09-20: they are hidden from the approval list and cannot be approved. On branch `fix/approve-request-55-58`; not merged, pushed or deployed yet.*
...
- [x] Seat cards show no dietary or notes boxes, and a seated person is shown as "Name (Family or group name)" so same-named people from different parties can be told apart (Next Action 63, owner request 2026-09-20). *Built, tested and click-tested on a stand-in database; not pushed or deployed yet. The spreadsheet download still has its dietary and notes sections — see TASKS.md Action 63.*
