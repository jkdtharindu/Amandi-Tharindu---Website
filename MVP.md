...
- [ ] **After Action 74 is deployed** (`40a2d09`, not pushed yet, and it must not go out alone — see Next Action 74): on `/admin/guests`, edit a party that has named people, add two people under "Add more people to this party", press **Save changes** (not the small green button), and confirm the list shows the new Seats number and reopening the party shows the new names. This has only been proven against a stand-in database.
...
- [x] Pending "add another person" requests from a removed guest still showed for approval (Next Action 58, found 2026-09-20). *Fixed and click-tested 2026-09-20: they are hidden from the approval list and cannot be approved. Also on its own clean branch as PR #16 (open, 3 of 3 checks passing); not merged or deployed yet.*
