...
Test suite on 2026-09-21: **606 of 606 pass** (16 new tests). The 9 failures that stood at the end of 2026-09-20 — all admin session and auth tests broken by the unpushed multi-admin work — were fixed under TASKS.md Action 69, along with two faults behind them and 16 new tests. Everything through Action 63 is pushed and deployed; Actions 55 and 58 (13 tests, passing) are committed inside that unpushed work.
...
- [x] **Multi-admin is pushed, deployed, and both admin logins are confirmed working on the live site (2026-09-22).** The owner ran migrations 020-022, then set `BRIDE_EMAIL`/`BRIDE_PASSWORD_HASH` and `GROOM_EMAIL`/`GROOM_PASSWORD_HASH` in Vercel Production. The first round of hashes silently failed: on Windows Command Prompt, `echo "text" | node scripts/set-admin-password.js` sends the quote marks through literally (CMD does not strip them, unlike PowerShell or a Unix shell), so the script hashed `"password"` — quotes included — while the owner typed the plain password at login. Every login attempt failed with no server-side clue why. Diagnosed by adding a temporary, non-secret diagnostic log (booleans and lengths only, deployed to a staged build first, never to the live domain) and confirmed by direct hash computation. Fixed by regenerating both hashes with no quotes in the command (`echo Bride12345678 | node scripts/set-admin-password.js`) and updating Vercel Production directly via `vercel env add <NAME> production` (stdin, non-interactive — avoids the environment-picker mistakes below). See MEMORY.md 2026-09-22 for the full writeup. **`npm run admin:set-password`'s own usage comment should be updated to warn Windows CMD users about this** (filed as a Next Action).
