...
| P0-02 | Personalized Digital Invitation | After login, guest sees a personalized invitation page. Admin uploads a base invitation image (JPG/PNG). Guest's name is overlaid via DOM (absolutely positioned CSS, no server-side image manipulation). Admin configures name position, font, size, color via admin panel. **Implemented 23 August 2026** — `/invitation/:code` renders the InvitationTemplate with the guest's name absolutely positioned from `ThemeSettings` (top, left, font size, colour), with a fallback card when no template is set. Prototype-scoped deviation: the template is supplied as a URL rather than a file upload (Supabase Storage not yet wired). |
